...
class LoadDataStep(PedicleScrewSimulatorStep):
    ...
    def createUserInterface( self ):
      ...
      # Back button to return to DummySpine step
      self.__backButton = qt.QPushButton("Back to Dummy")
      self.__layout.addRow(self.__backButton)
      self.__backButton.connect('clicked(bool)', self.backToDummy)
    ...
    def backToDummy(self):
      """Trigger the workflow's back action if available, otherwise set parameter and select module."""
      try:
        # try to find CTK workflow back button and click it
        bl = slicer.util.findChildren(text='Back')
        if len(bl):
          for b in bl:
            try:
              b.click()
      ...
      # Fallback: set parameter to DummySpine and reselect module so the UI resets to step 0
      try:
        pNode = self.parameterNode()
        if pNode:
          pNode.SetParameter('currentStep', 'DummySpine')
          slicer.util.selectModule('PedicleScrewSimulator')
      except Exception:
        pass
