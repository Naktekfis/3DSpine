...
class PedicleScrewSimulatorWidget(ScriptedLoadableModuleWidget):
  ...
  def setup(self):
    ...
    # Instantiate workflow controller
    ...
    # New dummy spine step goes before loading volumes
    self.dummySpineStep = PedicleScrewSimulatorWizard.DummySpineStep( 'DummySpine' )
    ...
    # Order: Dummy -> LoadData -> DefineROI -> Landmarks -> Measurements -> Screw -> Grade -> Final
    allSteps.append( self.dummySpineStep )
    ...
    # Add transitions
    self.workflow.addTransition( self.dummySpineStep, self.loadDataStep )
    # allow going back from LoadData to DummySpine
    self.workflow.addTransition( self.loadDataStep, self.dummySpineStep )
